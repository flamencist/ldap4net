...
using static LdapForNet.Native.LinuxNative;
...
namespace LdapForNet.Native
{
    internal abstract class LdapNative
    {
        ...
        private static LdapNative CreateInstance()
        {
            ...
            {
                return new LinuxLdapNative();
            }
            if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
            {
                return new MacOsLdapNative();
            }
    ...
    internal class LinuxLdapNative:LdapNative
    {
        ...
        internal override int ldap_set_option(SafeHandle ld, int option, ref int invalue) 
            => LinuxNative.ldap_set_option(ld, option, ref invalue);
        ...
        internal override int ldap_set_option(SafeHandle ld, int option, IntPtr invalue)
            => LinuxNative.ldap_set_option(ld, option,  invalue);


        internal override int ldap_get_option(SafeHandle ld, int option, ref string value) 
            => LinuxNative.ldap_get_option(ld, option, ref value);

        internal override int ldap_get_option(SafeHandle ld, int option, ref IntPtr value)
            => LinuxNative.ldap_get_option(ld, option, ref value);
        
        internal override int ldap_unbind_s(IntPtr ld) => LinuxNative.ldap_unbind_s(ld);
        ...
        internal override int ldap_search_ext(SafeHandle ld, string @base, int scope, string filter, string[] attrs, int attrsonly,
            ...
            LinuxNative.ldap_search_ext(ld, @base, scope, filter, attrs, attrsonly,
        ...
        internal override LdapResultType ldap_result(SafeHandle ld, int msgid, int all, IntPtr timeout, ref IntPtr pMessage) => 
            LinuxNative.ldap_result(ld,msgid,all,timeout,ref pMessage);
        ...
        internal override int ldap_parse_result(SafeHandle ld, IntPtr result, ref int errcodep, ref string matcheddnp, ref string errmsgp,
            ...
            LinuxNative.ldap_parse_result(ld, result, ref errcodep, ref matcheddnp, ref errmsgp, ref referralsp,
                ref serverctrlsp, freeit);

        internal override string LdapError2String(int error) => LinuxNative.LdapError2String(error);

        internal override string GetAdditionalErrorInfo(SafeHandle ld) => LinuxNative.GetAdditionalErrorInfo(ld);

        internal override int ldap_parse_reference(SafeHandle ld, IntPtr reference, ref string[] referralsp, ref IntPtr serverctrlsp, int freeit) => LinuxNative.ldap_parse_reference(ld, reference, ref referralsp, ref serverctrlsp, freeit);

        internal override IntPtr ldap_first_entry(SafeHandle ld, IntPtr message) => LinuxNative.ldap_first_entry(ld, message);

        internal override IntPtr ldap_next_entry(SafeHandle ld, IntPtr message) => LinuxNative.ldap_next_entry(ld, message);

        internal override IntPtr ldap_get_dn(SafeHandle ld, IntPtr message) => LinuxNative.ldap_get_dn(ld, message);

        internal override void ldap_memfree(IntPtr ptr) => LinuxNative.ldap_memfree(ptr);

        internal override void ldap_msgfree(IntPtr message) => LinuxNative.ldap_msgfree(message);

        internal override IntPtr ldap_first_attribute(SafeHandle ld, IntPtr entry, ref IntPtr ppBer) => LinuxNative.ldap_first_attribute(ld, entry, ref ppBer);

        internal override IntPtr ldap_next_attribute(SafeHandle ld, IntPtr entry, IntPtr pBer) => LinuxNative.ldap_next_attribute(ld, entry, pBer);

        internal override void ldap_value_free(IntPtr vals) => LinuxNative.ldap_value_free(vals);

        internal override IntPtr ldap_get_values(SafeHandle ld, IntPtr entry, IntPtr pBer) => LinuxNative.ldap_get_values(ld, entry, pBer);

        internal override int ldap_add_ext(SafeHandle ld, string dn, IntPtr attrs, IntPtr serverctrls, IntPtr clientctrls, ref int msgidp) => LinuxNative.ldap_add_ext(ld, dn, attrs, serverctrls, clientctrls, ref msgidp);

        internal override int ldap_modify_ext(SafeHandle ld, string dn, IntPtr mods, IntPtr serverctrls, IntPtr clientctrls, ref int msgidp) => LinuxNative.ldap_modify_ext(ld, dn, mods, serverctrls, clientctrls, ref msgidp);

        internal override int ldap_delete_ext(SafeHandle ld, string dn, IntPtr serverctrls, IntPtr clientctrls, ref int msgidp) => LinuxNative.ldap_delete_ext(ld, dn, serverctrls, clientctrls, ref msgidp);
        ...
        internal override int ldap_compare_ext(SafeHandle ld, string dn, string attr, IntPtr bvalue, IntPtr serverctrls, IntPtr clientctrls,
            ref int msgidp) =>
            LinuxNative.ldap_compare_ext(ld, dn, attr, bvalue, serverctrls, clientctrls, ref msgidp);
        ...
        internal override int ldap_rename(SafeHandle ld, string dn, string newrdn, string newparent, int deleteoldrdn, IntPtr serverctrls,
            ...
            LinuxNative.ldap_rename(ld, dn, newrdn, newparent, deleteoldrdn, serverctrls, clientctrls, ref msgidp);

        internal override void ldap_controls_free(IntPtr ctrls) => LinuxNative.ldap_controls_free(ctrls);
    }
    
    internal class MacOsLdapNative:LdapNative
    {
        internal override int Init(ref IntPtr ld, string hostname, int port) => 
            MacOsNative.ldap_initialize(ref ld,$"LDAP://{hostname}:{port}");
        ...
        internal override int BindKerberos(SafeHandle ld)
        {
            ...
            return MacOsNative.ldap_sasl_interactive_bind_s(ld, null, LdapAuthMechanism.GSSAPI, IntPtr.Zero, IntPtr.Zero,
                (uint)LdapInteractionFlags.LDAP_SASL_QUIET, (l, flags, d, interact) => (int)LdapResultCode.LDAP_SUCCESS, ptr);
        }
        ...
        internal override async Task<IntPtr> BindKerberosAsync(SafeHandle ld)
        {
            var task = Task.Factory.StartNew(() =>
            {
                ...
                do
                {
                    rc = MacOsNative.ldap_sasl_interactive_bind(ld, null, LdapAuthMechanism.GSSAPI, IntPtr.Zero, IntPtr.Zero,
                        (uint) LdapInteractionFlags.LDAP_SASL_QUIET,
                        SaslInteractProc , ptr, result, ref rmech,
                        ref msgid);
                    if (rc != (int) LdapResultCode.LDAP_SASL_BIND_IN_PROGRESS)
                    ...
                    MacOsNative.ldap_msgfree(result);

                    if (MacOsNative.ldap_result(ld, msgid, 0, IntPtr.Zero, ref result) == LdapResultType.LDAP_ERROR)
                    {
                        ThrowIfError(rc,nameof(MacOsNative.ldap_sasl_interactive_bind));
                    }
                ...
                ThrowIfError(ld,rc, nameof(MacOsNative.ldap_sasl_interactive_bind), new Dictionary<string, string>
                {
                    [nameof(saslDefaults)] = saslDefaults.ToString()
                });
                return result;
        ...
        internal override int BindSimple(SafeHandle ld, string userDn, string password) =>
            MacOsNative.ldap_simple_bind_s(ld, userDn, password);
        ...
        internal override async Task<IntPtr> BindSimpleAsync(SafeHandle _ld, string userDn, string password)
        {
            ...
            return await Task.Factory.StartNew(() =>
            {
                ...
                var msgidp = 0;
                var result = IntPtr.Zero;
                MacOsNative.ldap_sasl_bind(_ld, userDn, null, ptr, IntPtr.Zero, IntPtr.Zero, ref msgidp);
                if (msgidp == -1)
                {
                    throw new LdapException($"{nameof(BindSimpleAsync)} failed. {nameof(MacOsNative.ldap_sasl_bind)} returns wrong or empty result",  nameof(MacOsNative.ldap_sasl_bind), 1);
                }

                var rc = MacOsNative.ldap_result(_ld, msgidp, 0, IntPtr.Zero, ref result);

                if (rc == LdapResultType.LDAP_ERROR || rc == LdapResultType.LDAP_TIMEOUT)
                {
                    ThrowIfError((int)rc,nameof(MacOsNative.ldap_sasl_bind));
                }
        ...
        internal override int ldap_set_option(SafeHandle ld, int option, ref int invalue) 
            => MacOsNative.ldap_set_option(ld, option, ref invalue);

        internal override int ldap_set_option(SafeHandle ld, int option, ref string invalue)=>
            MacOsNative.ldap_set_option(ld, option, ref invalue);

        internal override int ldap_set_option(SafeHandle ld, int option, IntPtr invalue)
            => MacOsNative.ldap_set_option(ld, option,  invalue);


        internal override int ldap_get_option(SafeHandle ld, int option, ref string value) 
            => MacOsNative.ldap_get_option(ld, option, ref value);

        internal override int ldap_get_option(SafeHandle ld, int option, ref IntPtr value)
            => MacOsNative.ldap_get_option(ld, option, ref value);
        
        internal override int ldap_unbind_s(IntPtr ld) => MacOsNative.ldap_unbind_s(ld);

        internal override int ldap_search_ext(SafeHandle ld, string @base, int scope, string filter, string[] attrs, int attrsonly,
            IntPtr serverctrls, IntPtr clientctrls, IntPtr timeout, int sizelimit, ref int msgidp) =>
            MacOsNative.ldap_search_ext(ld, @base, scope, filter, attrs, attrsonly,
                serverctrls, clientctrls, timeout, sizelimit, ref msgidp);

        internal override LdapResultType ldap_result(SafeHandle ld, int msgid, int all, IntPtr timeout, ref IntPtr pMessage) => 
            MacOsNative.ldap_result(ld,msgid,all,timeout,ref pMessage);

        internal override int ldap_parse_result(SafeHandle ld, IntPtr result, ref int errcodep, ref string matcheddnp, ref string errmsgp,
            ref IntPtr referralsp, ref IntPtr serverctrlsp, int freeit) =>
            MacOsNative.ldap_parse_result(ld, result, ref errcodep, ref matcheddnp, ref errmsgp, ref referralsp,
                ref serverctrlsp, freeit);

        internal override string LdapError2String(int error) => MacOsNative.LdapError2String(error);

        internal override string GetAdditionalErrorInfo(SafeHandle ld) => MacOsNative.GetAdditionalErrorInfo(ld);

        internal override int ldap_parse_reference(SafeHandle ld, IntPtr reference, ref string[] referralsp, ref IntPtr serverctrlsp, int freeit) => MacOsNative.ldap_parse_reference(ld, reference, ref referralsp, ref serverctrlsp, freeit);

        internal override IntPtr ldap_first_entry(SafeHandle ld, IntPtr message) => MacOsNative.ldap_first_entry(ld, message);

        internal override IntPtr ldap_next_entry(SafeHandle ld, IntPtr message) => MacOsNative.ldap_next_entry(ld, message);

        internal override IntPtr ldap_get_dn(SafeHandle ld, IntPtr message) => MacOsNative.ldap_get_dn(ld, message);

        internal override void ldap_memfree(IntPtr ptr) => MacOsNative.ldap_memfree(ptr);

        internal override void ldap_msgfree(IntPtr message) => MacOsNative.ldap_msgfree(message);

        internal override IntPtr ldap_first_attribute(SafeHandle ld, IntPtr entry, ref IntPtr ppBer) => MacOsNative.ldap_first_attribute(ld, entry, ref ppBer);

        internal override IntPtr ldap_next_attribute(SafeHandle ld, IntPtr entry, IntPtr pBer) => MacOsNative.ldap_next_attribute(ld, entry, pBer);

        internal override void ldap_value_free(IntPtr vals) => MacOsNative.ldap_value_free(vals);

        internal override IntPtr ldap_get_values(SafeHandle ld, IntPtr entry, IntPtr pBer) => MacOsNative.ldap_get_values(ld, entry, pBer);

        internal override int ldap_add_ext(SafeHandle ld, string dn, IntPtr attrs, IntPtr serverctrls, IntPtr clientctrls, ref int msgidp) => MacOsNative.ldap_add_ext(ld, dn, attrs, serverctrls, clientctrls, ref msgidp);

        internal override int ldap_modify_ext(SafeHandle ld, string dn, IntPtr mods, IntPtr serverctrls, IntPtr clientctrls, ref int msgidp) => MacOsNative.ldap_modify_ext(ld, dn, mods, serverctrls, clientctrls, ref msgidp);

        internal override int ldap_delete_ext(SafeHandle ld, string dn, IntPtr serverctrls, IntPtr clientctrls, ref int msgidp) => MacOsNative.ldap_delete_ext(ld, dn, serverctrls, clientctrls, ref msgidp);

        internal override int ldap_compare_ext(SafeHandle ld, string dn, string attr, IntPtr bvalue, IntPtr serverctrls, IntPtr clientctrls,
            ref int msgidp) =>
            MacOsNative.ldap_compare_ext(ld, dn, attr, bvalue, serverctrls, clientctrls, ref msgidp);

        internal override int ldap_rename(SafeHandle ld, string dn, string newrdn, string newparent, int deleteoldrdn, IntPtr serverctrls,
            IntPtr clientctrls, ref int msgidp) =>
            MacOsNative.ldap_rename(ld, dn, newrdn, newparent, deleteoldrdn, serverctrls, clientctrls, ref msgidp);

        internal override void ldap_controls_free(IntPtr ctrls) => MacOsNative.ldap_controls_free(ctrls);
    }
}